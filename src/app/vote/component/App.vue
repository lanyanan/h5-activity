  ...
  export default {
    ...
    created (){
      ...
      request.doGet('/router?method=jz.h5.interact.hot.index', {id: activeId}).then(function(res){
        let resObj = res.obj;
        //活动页面标题
        document.title = resObj.title || '测测你是学霸还是学渣';
        that.appbgc = resObj.backgroundColor || '#232a2f';
...
<style>
  ...
  .wrap .bgm{position: absolute;top: 0.714286rem;right:0.714286rem;width:2.142857rem;height:2.142857rem;background:url('../assets/bgm.png') no-repeat;background-size: 100%;}
