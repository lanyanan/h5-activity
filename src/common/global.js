import $ from '../../lib/jquery/jquery-1.11.3.min.js';
/*
 *  date format
 */
...
(function($){
	var ConfirmRe = function(options){
		...
		this.confirm = options.confirm || '确定';
	...
	ConfirmRe.prototype = {
		init:function(options){
			var $_this = this;
			$('body').append(this.createTags(this.msg));
		...
		createTags:function(msg){
			...
			+						'<div class="'+ this.confirmButton +'" style="'+ buttonCss +'background:#e53c5f;display:block; -webkit-box-flex:1;">确认</div>'
			+						'<div class="'+ this.cancleButton +'" style="'+ buttonCss +'background:#b3b3b3;display:block; -webkit-box-flex:1; margin-left:10px;">取消</div>'
			+					'</div>'
			+				'</div>'
			+			'</div>';
			return tags;
